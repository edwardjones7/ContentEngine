...
import { normalizeGoal, normalizeFunnel } from './stages.mjs';
...
export function ideatePrompt({ count = 6, avoid = [] } = {}) {
  ...
  return {
    ...
    user: `Propose ${count} distinct content ideas grounded in current pain points of US service-business owners (contractors, HVAC, plumbers). Vary the approach across ideas (diagnostic teardown / contrarian take / specific playbook), but "angle" must be a 1-2 sentence editorial angle stating the specific argument the post makes — never a category label.${avoidLine} Return JSON: {"ideas":[{"id","title","angle","hook","source":"web"|"brand","goal":"leads"|"authority"|"nurture"|"story"|"values","funnel":"tof"|"mof"|"bof"}]}. "goal" is the business job of the post: leads (demand capture), authority (expertise proof), nurture (educate the existing audience), story (narrative/case), values (POV/beliefs). "funnel" is the audience stage: tof (top — broad awareness), mof (middle — evaluating solutions), bof (bottom — ready to buy). Hooks in Elenos voice — short, specific, no hype.`,
  };
}
export const postIdeate = (out) => {
  ...
  return ideas.map((i) => ({ ...i, goal: normalizeGoal(i.goal), funnel: normalizeFunnel(i.funnel) }));
};
