// Dead-simple JSON store. The interface (getPieces/savePiece/…) is what ports to
// Postgres in the monorepo; only the implementation changes.
...
export function addPublished(post) {
  ...
  if (i >= 0) db.published[i] = post;
  else db.published.unshift(post);
  write(db);
  return post;
}
